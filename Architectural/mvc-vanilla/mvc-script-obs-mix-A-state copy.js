...
let ProductControls = (function() {
    ...
    function View(id, ctrl, opts) {
        ...
        this.ctrl.handle('click',"produvtControls:update", this.$heading, {keyCode: 64, data: "haha"});
        ...
        this.update = function(data) {
            this.ctrl.getContainer().find('#' + this.id + "Name").val(data.name);
            this.ctrl.getContainer().find('#' + this.id + "Schema").val(data.schemaName);
        }
    }
...
Controller.prototype.handle = function(eventType, eventName, $selector, options) { //функция создания события ->rf:factory
    ...
    if(eventName == "caption:update")
    {
        ...
        $selector.on(eventType, { data: data }, 
            (e) => this.getContainer().trigger(evt, e));
    }
