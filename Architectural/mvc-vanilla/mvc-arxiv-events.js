...
let ProductControls = (function() {
    ...
    function Model(ctrl) {
        ...
        this.notify = function (obj) {
            this.ctrl.observers.forEach(function (observer) {
                observer.update(obj); //???self
            });
        }
    }
    ...
    function View(id, ctrl, opts) {
        ...
        this.update = function(data) {
            console.log(data.nm, data.sch);
            this.ctrl.getContainer().find('#' + this.id + "Name").val(data.nm);
            this.ctrl.getContainer().find('#' + this.id + "Schema").val(data.sch);
        }
    }
    ...
    function ProductControls($container) {
        ...
        this.setName = function(val) {
            model.productNm = val;
            model.notify({nm: model.productNm});
        }

        this.setSch = function(val) {
            model.schNm = val;
            model.notify({sch: model.schNm});
        }    
...
function Model(id, ctrl) {
    ...
    this.updateRelated = function(val) {
        if(Array.isArray(this.ctrl.colleagues.relatedComponents))
        {
            console.log("Notification of related models started.");
            this.ctrl.colleagues.relatedComponents.forEach(function(m) {
                console.log("Имя модели: " + m.name, "Значение модели " + val);
                m.component.setSch(val);
                m.component.setName(val);
            })
            console.log("Зависимые модели обновлены");
        }
    }
    ...
    this.changeHeading = function() {
        ...
        this.updateRelated(state.getValue());
    }
...
function Arxiv($doc) {
    ...
    this.run = () => this.observers.forEach(element => {
        let rendered = element.render();
        this.colleagues.relatedComponents = [];
        this.colleagues.relatedComponents[0] = {
            name: "ProductControls",
            component: new ProductControls(rendered.$productControls)
        };
        // this.observe(ctrlProduct.getView()); //Вариант подписки Представления
        this.colleagues.relatedComponents[0].component.run();
